...
//    val num = sum((yDiff :* yDiff).apply(*, ::))
    ...
    i.indices.foreach(idx => num.unsafeUpdate(idx, idx, 0.0))
  ...
  def compute(
    ...
    val n = Y.rows
    val m = data.length
    val p = new CSCMatrix[Double](
      data.flatMap(_._2.map(_._2)),
      m, n,
      data.flatMap{case (i, itr) => itr.map(_=>i)},
      data.flatMap(_._2.map(_._1)))
      .toDenseMatrix
      .mapValues(x => math.max(if(exaggeration) x * 4 else x, 1e-12))
